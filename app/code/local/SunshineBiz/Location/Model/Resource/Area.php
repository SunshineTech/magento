...
class SunshineBiz_Location_Model_Resource_Area extends SunshineBiz_Locale_Model_Resource_Abstract {
    ...
    protected function _construct() {
        ...
        $this->_init('location/area', 'id');
        $this->_nameTable = $this->getTable('location/area_name');
    ...
    protected function _beforeSave(Mage_Core_Model_Abstract $area) {
        ...
        if (!$area->getId()) {
            $area->setCreatedAt(Mage::getSingleton('core/date')->gmtDate());
        }
        
        $area->setUpdatedAt(Mage::getSingleton('core/date')->gmtDate());
    ...
    public function getNameCols() {
        return array('name', 'abbr', 'mnemonic');
    }
    ...
    public function isUniqueLocaleName(Mage_Core_Model_Abstract $area, $name) {
        ...
        $parent = Mage::getModel('location/area')->setLocale($name['locale'])->load($area->getParentId());
