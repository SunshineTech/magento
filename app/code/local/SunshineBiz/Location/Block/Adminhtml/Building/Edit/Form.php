 ...
 */
class SunshineBiz_Location_Block_Adminhtml_Building_Edit_Form extends SunshineBiz_Location_Block_Adminhtml_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        $fieldset->addField('area_id', 'select', array(
            'name' => 'area_id',
            'label' => $this->_helper->__('Area'),
            'id' => 'area_id',
            'title' => $this->_helper->__('Area'),
            'class' => 'input-select',
            'onchange' => 'locationChanged(this, \'' . $this->getUrl('*/json/areaCircle') . '\',  \'building_circle_id\')',
            'required' => true,
        ))->setValues($options);
        ...
        if ($model->getAreaId()) {
            $options = Mage::getResourceModel('location/area_collection')
                    ->addFieldToFilter('parent_id', $model->getAreaId())
                    ->addFieldToFilter('type', SunshineBiz_Location_Model_Area::TYPE_TRADE_CIRCLE)
                    ->load()
                    ->toOptionArray();
        }
        $fieldset->addField('circle_id', 'select', array(
            'name' => 'circle_id',
            'label' => $this->_helper->__('Trade Circle'),
            'id' => 'circle_id',
            'title' => $this->_helper->__('Trade Circle'),
            'class' => 'input-select',
        ))->setValues($options);
