...
/**
 * SunshineBiz_Admin rules resource
 *
 ...
 */
class SunshineBiz_Admin_Model_Resource_Rules extends Mage_Admin_Model_Resource_Rules {
    ...
    protected function saveUserRules(Mage_Admin_Model_Rules $rule) {
        ...
        $allowResources = Mage::helper('sunshinebiz_admin')->getAllowResources($user); //角色现存的权限规则的资源
        $userResources = array();
        $denyResources = array();
        ...
        $adapter = $this->_getWriteAdapter();
        $adapter->beginTransaction();
        $userRuleTable = $this->getTable('sunshinebiz_admin/user_rule');
        try {
            if ($insertResources) {
                $row = array(
                    'privileges' => '', // not used yet
