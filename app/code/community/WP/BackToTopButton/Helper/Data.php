...
class WP_BackToTopButton_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function isDisabled() {
        if (!Mage::getStoreConfigFlag('design/backtotopbutton/enabled'))
            return true;
        // --- to disable by regexp expression ---
        if ($this->isDisabledByUserAgentAgainstRegexps())
            return true;
        return false;
    }

    public function isDisabledByUserAgentAgainstRegexps() {
        $regexpsConfigPath = 'design/backtotopbutton/disabled_by_regexp';
