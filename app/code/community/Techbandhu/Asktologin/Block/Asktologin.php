...
class Techbandhu_Asktologin_Block_Asktologin extends Mage_Core_Block_Template
{
	...
	public function canAccess()
	{
	    ...
	    if(!Mage::getSingleton("customer/session")->isLoggedIn())
	    {
		    ...
		    if(Mage::getStoreConfig('customer/asktologin/if_new')){
	...
	{
		$shouldAsk = false;
		$config = Mage::getStoreConfig('customer/asktologin/is_active');
		if($config){
	...
	public function newCustomer(){
		$data = "false";
		$config = Mage::getStoreConfig('customer/asktologin/if_new');
		if($config){
